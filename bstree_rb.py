...
class RBTNode:
    ...
    def __eq__(self, other):
        return isinstance(other, RBTNode)\
    ...
    def __repr__(self):
        return "RBTNode(k=%s, v=%s, left=%s, right=%s, color=%s)"\
        % (self.key, self.val, self.left, self.right, self.color)
    #---- to do ----
...
def get(tree, key):
    ...
    Args:
        tree (RBTNode): the root of TreeMap
...
def rebalance(tree):
    ...
    and tree.left.left and tree.left.left.color == 'R':
        ...
        return RBTNode(tree.left.key, tree.left.val,\
        RBTNode(tree.left.left.key, tree.left.left.val, tree.left.left.left, tree.left.left.right, 'B'),\
        RBTNode(tree.key, tree.val, tree.left.right, tree.right))
...
def insert_helper(tree, key, val):
    ...
    Args:
        tree (RBTNode): the root of the BST tree/subtree
    ...
    """
    if tree is None:
        return RBTNode(key, val, None, None, 'R')
    if key == tree.key:
        tree.val = val
    elif key < tree.key:
